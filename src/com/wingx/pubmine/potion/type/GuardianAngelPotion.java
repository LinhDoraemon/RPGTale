...
public class GuardianAngelPotion extends CustomPotion {
	...
	public static void onApply(PlayerItemConsumeEvent e) {
		if (!GUARDIAN_ANGEL_LIST.contains(e.getPlayer().getUniqueId())) {
			...
			new CountdownBossbar().showBossbar("§fHộ mạng §a[00:15]", org.bukkit.boss.BarColor.GREEN, BarStyle.SOLID,
					15, new Player[] { e.getPlayer() });
			Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Bukkit.getPluginManager().getPlugin("RPGTale"),
					new Runnable() {
						public void run() {
							if (GuardianAngelPotion.GUARDIAN_ANGEL_LIST.contains(e.getPlayer().getUniqueId())) {
								GuardianAngelPotion.GUARDIAN_ANGEL_LIST.remove(e.getPlayer().getUniqueId());
							}
						}
					}, 300L);
		} else {
	...
	public static void onActivate(EntityDamageByEntityEvent e) {
		...
		if ((p.getHealth() - e.getFinalDamage() <= 0.0D) && (GUARDIAN_ANGEL_LIST.contains(p.getUniqueId()))) {
			e.setCancelled(true);
			...
			Bukkit.getServer().getScheduler().scheduleSyncDelayedTask(Bukkit.getPluginManager().getPlugin("RPGTale"),
					new Runnable() {
						public void run() {
							loc.getWorld().spawnParticle(Particle.VILLAGER_HAPPY, loc.add(0.0D, 0.0D, 0.0D), 100, 0.3D,
									0.3D, 0.3D);
							p.setHealth(10.0D);
							p.setFlySpeed(0.3F);
